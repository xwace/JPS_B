...
//--------------------------------------------------------------------
// Code : GetCloseValue
// Desc : x, y ��ǥ���� x or y �� forward(+) or backward(-) �������� �˻�
//--------------------------------------------------------------------
...
//#ifdef DEBUG1
        {
            auto  pos = xaxis ? _GetPosX(x , y) : _GetPosY(x, y );
            if (pos == npos) return maxvalue;

            int col = variable, obs_col{maxvalue};
            auto curId = std::div(variable, 64);
            for (int i = curId.quot; i < MAX + 1; ++i) {
                pos = xaxis ? _GetPosX(x + i * nbitmask, y) : _GetPosY(x, y + i * nbitmask);
			    if (pos == npos) return maxvalue;

                unsigned long long v;
                v = (i == curId.quot) ? (maskbound[pos / nbitmask] >> (curId.rem + 1)) : (maskbound[pos / nbitmask]);
                auto distance = __builtin_ctzll(v);

                if (distance != 64) {
                    obs_col = (i == curId.quot) ? col + distance + 1 : i * 64 + distance;
                    break;
                }
            }
            maxvalue = obs_col;

            for (int i = 0; i < 300; ++i) {std::cout<<(int)(DEBUG::src_c1).row(y).at<uchar>(0,i);}std::cout<<std::endl;
            std::cout<<"cmp: "<<maxvalue<<std::endl;
            return maxvalue;
        }
...
//#else
		...
		for (int i = 0; i < MAX; ++i)
		{
			...
			{
                std::cout<<"cmppppppppppppp: "<<((variable - (variable%nbitmask) + i * nbitmask) + index)<<std::endl;
				return maxvalue;
...
//--------------------------------------------------------------------
// Code : _flsll
// Desc : _BitScanReverse64 �� ������ �ڵ� (Windows, Android, IOS ��� ������ �ڵ带 ����)
//--------------------------------------------------------------------
...
//--------------------------------------------------------------------
// Code : _ffsll
// Desc : _BitScanForward64 �� ������ �ڵ� (Windows, Android, IOS ��� ������ �ڵ带 ����)
//--------------------------------------------------------------------
bool JPSCollision::_ffsll(unsigned long& index, UInt64 word)
{
//    std::cout<<"input\n"<<std::bitset<64>{word}<<std::endl;
...
//        std::cout<<"index 1\n"<<std::bitset<64>{word}<<std::endl;
	...
	index = num;
//    std::cout<<"done one loop: "<<index<<std::endl;getchar();
